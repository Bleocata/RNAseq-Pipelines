### VLIRA JUN2024 -2024-06-14 ####
# ==============================================================================#
# Nesse script utiliza como input os resultados de "Pipeline_Mutect2.GATK4.6_2024-06-14" 
# 
# Dataset reorganization 
# LOAD CLINICAL DATA : Seleção das 71 amostras LARC
...
# Utilização do Maftools para salvar tabela de mutação no formato .MAF
...
setwd("C:/Users/beatr/Sirio Libanes")
input <- read_tsv("Final_GATK.4.6_annotated.tsv")
...
dim(all_variants) #123134     49

#==============================================================================#
# LOAD CLINICAL DATA ####
#==============================================================================#
Clinical <- xlsx::read.xlsx("D:/PROJETOS-HSL_BP/tabelas materiais biologicos e clinicos/Dados Clinicos 2024 ALLversions WXS-TCRseq.xlsx", sheetName = "73samples", header = T)
...
Clinical <- Clinical[Clinical$EXOME_ID != "ROP-128",]
...
length(unique(all_variants.SelectSamples$EXOME_ID)) # 71  #72
length(unique(all_variants.SelectSamples$index))   #79346  #72-> 79558
...
GENOTYPE.dt <- as.data.frame(all_variants.SelectSamples$GENOTYPE)
GENOTYPE.dt<- separate(data = GENOTYPE.dt, col = 1, into=FORMAT, sep = ":")

## Separate Alleles ###
GENOTYPE.dt$aux = GENOTYPE.dt$GT
GENOTYPE.dt$aux = str_replace(GENOTYPE.dt$aux,"/","|")   
GENOTYPE.dt<- separate(data = GENOTYPE.dt, col = aux, into=c('GT1','GT2'), sep = "\\|")
GENOTYPE.dt$aux = GENOTYPE.dt$AD
GENOTYPE.dt<- separate(data = GENOTYPE.dt, col = aux, into=c('AD1','AD2'), sep = ",")

all_variants.SelectSamples<-cbind(all_variants.SelectSamples, GENOTYPE.dt[,c("GT","AD","GT1","GT2","AD1","AD2")])
...
dim(all_var.SelectSamples)  #115183     68
length(unique(all_var.SelectSamples$index)) #79346   #72-> 79558
## Quantifica MUTAÇOES GERMINATIVAS 
Germline_gnomad <- which(all_var.SelectSamples$gnomad41_exome_AF_grpmax > 0.0005)
nrow(unique(all_var.SelectSamples[Germline_gnomad,"index"])) #8689  #72-> 8723 nao-redundate germinativos GnomAD
## Quantifica MUTAÇOES GERMINATICAS ABRAOM 
Germline_abraom <- which(all_var.SelectSamples$abraom_freq > 0.0005)
nrow(unique(all_var.SelectSamples[Germline_abraom,"index"])) #6988  #72-> 7006 nao-redundate germinativos Abraom

## Quantifica MUTAÇOES not-rare gnomAD
not_rare2exclude <- which(all_var.SelectSamples$gnomad41_exome_AF_grpmax > 0.0005)
length(not_rare2exclude)       # 15071     #72-> 15193 excluidos
nrow(unique(all_var.SelectSamples[not_rare2exclude,"index"]))  # 8689 #72-> 8723 nao-redundate excluidos
mut.GF1 <- all_var.SelectSamples[-c(not_rare2exclude),]
length(unique(mut.GF1$index))     # 70657  #72-> 70835 variantes ficaram

## Quantifica MUTAÇOES not-rare no Abraom
not_rareAbraom2exclude <- which(mut.GF1$abraom_freq > 0.0005)
length(not_rareAbraom2exclude)        # 2115    #72-> 2127 excluidos
nrow(unique(mut.GF1[not_rareAbraom2exclude,"index"])) # 1802  #72-> 1811 nao-redundate excluidos
mut.GF2 <- mut.GF1[-c(not_rareAbraom2exclude),]
length(unique(mut.GF2$index))          # 68855  #72-> 69024 variantes restantes
somatic_Var <- mut.GF2

# Selecionando ads Germinativa
germline_Var <- fsetdiff(all_var.SelectSamples, somatic_Var)
dim(all_var.SelectSamples)                  # 115.183  antes dos filtros temos esse total de variantes 
length(unique(germline_Var$index))          # 10491   variantes germinativas distintas 
dim(germline_Var)                           # 17.186 variantes germinativas removidas
fwrite(germline_Var, paste0(output,"/germline_variants.tsv"), quote = F, sep="\t")
length(unique(somatic_Var$index))           # 68855  #72-> 69024 variantes somaticas distintas
dim(somatic_Var)                            # 97.997  variantes somaticas selecionadas
...
whitelist$Chr <- paste0("chr", whitelist$Chr)

library(GenomicRanges)
gr1 <- GRanges(seqnames = somatic_Var$Chr,
               ranges = IRanges(start = as.numeric(somatic_Var$Start), end = as.numeric(somatic_Var$End)))
gr2 <- GRanges(seqnames = whitelist$Chr,
               ranges = IRanges(start = whitelist$Start, end = whitelist$End))

# Encontrar intersecções (similar ao bedtools intersect)
# NÃO EXISTEM VARIANTES GERMINATIVAS QUE ESTÃO EM REGIÕES DA WHITELIST
overlaps <- findOverlaps(gr1, gr2)

# Obter as linhas de `mut.filt` que intersectam com `whitelist`
mut.filt_intersect <- somatic_Var[queryHits(overlaps), ]

somatic_Var$Hotspotflag = 0
somatic_Var[somatic_Var$index %in% mut.filt_intersect$index, "Hotspotflag"] <- 1


## FILTRO: REMOVE mutações compartilhadas >=2 e <= 3 ocorrencias no COSMIC ####
#==============================================================================#
somatic_Var<-as.data.frame(fread(paste0(output,"somatic_variants.tsv"), header = T,  na.strings=c("NA"), fill=TRUE, check.names = FALSE))
length(unique(somatic_Var$index)) # 68855  variantes somaticas
dim(somatic_Var)                  # 97997    

shared2exclude <- which( somatic_Var$MUTATION_shared >= 2 & somatic_Var$COSMIC_count <= 3)
length(unique(somatic_Var[shared2exclude,"index"]))   # 8449  distintas
length((somatic_Var[shared2exclude,"index"]))   # 36530 variantes removidas
somatic_Var.Filt_ShareCosmic <- somatic_Var[-shared2exclude,]  
length(unique(somatic_Var.Filt_ShareCosmic[,"index"]))  # 60406 
length(unique(somatic_Var.Filt_ShareCosmic[,"SAMPLE"]))  #71      
dim(somatic_Var.Filt_ShareCosmic)
...
#==============================================================================#
#                   DISTRIBUICAO DAS VAF NAS 71 AMOSTRAS  ####
#==============================================================================#

somatic_Var.Filt_SC<-as.data.frame(fread(paste0(output,"somatic_variants.FiltSharedCosmic.tsv"), header = T,  na.strings=c("NA"), fill=TRUE, check.names = FALSE))
length(unique(somatic_Var.Filt_SC$index)) # 60406  #72-> 60568
dim(somatic_Var.Filt_ShareCosmic)
...
#==============================================================================#
genesHLA <- fread("D:/Github-Projects/Pipeline_Gatk-Mutect2/input_Somatic-Filter/genesHLA.txt", col.names = "gene", header = F) 
all.hla<- subset(somatic_Var.Filt_SC, (Gene.refGene %in% genesHLA$gene))
length(unique(all.hla$index))    # 52 var em hla

hla2exclude <- which(somatic_Var.Filt_SC$Gene.refGene %in% genesHLA$gene) 
length(unique(somatic_Var.Filt_SC[hla2exclude,"index"]))  #  52 var removidas
somatic_Var.Filt_SC_HLA <- somatic_Var.Filt_SC[-hla2exclude,]
length(unique(somatic_Var.Filt_SC_HLA$index)) #60354  
dim(somatic_Var.Filt_SC_HLA)    #61413    60

## FILTRO: REMOVE MUTACOES  MINOR ALLELIC FREQ. < 0.10 ####
#==============================================================================#
VAF2exclude <- which(somatic_Var.Filt_SC_HLA$VAF<0.1) 
length(unique(somatic_Var.Filt_SC_HLA[VAF2exclude,"index"]))  # 25221  
somatic_Var.Filt_SC_HLA_VAF <- somatic_Var.Filt_SC_HLA[-VAF2exclude,]
length(unique(somatic_Var.Filt_SC_HLA_VAF$index))  # 35187  
dim(somatic_Var.Filt_SC_HLA_VAF)                   # 35326    

## FILTRO: REMOVE MUTACOES COV < 30 ####
#==============================================================================#
cov2exclude <- which(somatic_Var.Filt_SC_HLA_VAF$COV<30) 
length(unique(somatic_Var.Filt_SC_HLA_VAF[cov2exclude,"index"])) # 1503 
somatic_Var.Filt_SC_HLA_VAF_COV <- somatic_Var.Filt_SC_HLA_VAF[-cov2exclude,]
length(unique(somatic_Var.Filt_SC_HLA_VAF_COV$index))  # 33691
dim(somatic_Var.Filt_SC_HLA_VAF_COV)              # 33817    60
...
length(unique(somatic_Var.CodRegion$index))  # 33536 
unique(somatic_Var.CodRegion$Func.refGene)   # "exonic"          "exonic;splicing" "splicing"  
dim(somatic_Var.CodRegion)                   # 33662    60
...
length(unique(syn.Mutation$index))  # 33536
dim(syn.Mutation)                   # 33662    60
...
#==============================================================================#
#  REMOVE MUTACOES CLASSIFICADAS COM SINONIMAS (synonymous SNV)   
nonsyn.Mutation = subset(somatic_Var.CodRegion, !(ExonicFunc.refGene %in% c("synonymous SNV")))
length(unique(nonsyn.Mutation[,"index"]))   # 25459 
...
length(unique(syn.Mutation.MAF$index)) # 33536 
...
Mb <- 34126765/1000000
#Mb=34.126
tmbSamples <- as.data.frame(table(nonsyn.Mutation$SAMPLE))
...
# POLE / POLD1
#==============================================================================#
dt.POLE_POLD1 <- nonsyn.Mutation %>%
  subset(Gene.refGene %in% c("POLE", "POLD1"))


#==============================================================================#
## Gene class (none, driver, dMMMR) 
#==============================================================================#
driver<-fread("D:/Github-Projects/Pipeline_Gatk-Mutect2/input_Somatic-Filter/driver-genes.txt", header = F, col.names = "gene")
MMR<-fread("D:/Github-Projects/Pipeline_Gatk-Mutect2/input_Somatic-Filter/MMR_genes.txt", header = F, col.names = "gene")
# Gerado pelo ChatGPT
dna_repair_genes = c(
...
Human.DNA.Repair.Genes<-fread("D:/Github-Projects/Pipeline_Gatk-Mutect2/input_Somatic-Filter/Human_dna_repair_genes_DB.tsv", header =  T)

#==============================================================================#
# Samples_Data
#==============================================================================#
nonsyn.Mutation.MAF <- syn.Mutation.MAF %>% filter(Variant_Classification != "Silent")
Samples_Data.1<- merge(Clinical[,c(1,2,9:29)], tmbSamples, by="Sample")
Samples_Data.1$POLE.POLD1 <-0
samplesPOLE.POLD1<- unique(subset(nonsyn.Mutation.MAF, Hugo_Symbol %in% c("POLE","POLD1"), select = SAMPLE))
Samples_Data.1[Samples_Data.1$Sample %in% samplesPOLE.POLD1$SAMPLE, "POLE.POLD1"] <- 1

dMMR <-(subset(Human.DNA.Repair.Genes, DNA_Repair.CLASS == "dMMR", select="DNA_Repair.GENE" ))

Samples_Data.1$dMMR <-0
samples.dMMR<- unique(subset(nonsyn.Mutation.MAF, Hugo_Symbol %in% dMMR$DNA_Repair.GENE, select = SAMPLE))
Samples_Data.1[Samples_Data.1$Sample %in% samples.dMMR$SAMPLE, "dMMR"] <- 1

#==============================================================================#
#Samples_Data.1<- merge(Clinical[,c(1,2,9:29)], tmbSamples, by="Sample")
Samples_Data.1$POLE <-0
samplesPOLE<- unique(subset(syn.Mutation.MAF, Hugo_Symbol == "POLE", select = Tumor_Sample_Barcode))
Samples_Data.1[Samples_Data.1$EXOME_ID %in% samplesPOLE$Tumor_Sample_Barcode, "POLE"] <- 1

Samples_Data.1$HDRG.ALL <-0
samplesHDRG<- unique(subset(syn.Mutation.MAF, Hugo_Symbol %in% Human.DNA.Repair.Genes$DNA_Repair.GENE, select = Tumor_Sample_Barcode))
Samples_Data.1[Samples_Data.1$EXOME_ID %in% samplesHDRG$Tumor_Sample_Barcode, "HDRG.ALL"] <- 1
# Obter todas as classes únicas de reparo de DNA
dna_repair_classes <- unique(Human.DNA.Repair.Genes$DNA_Repair.CLASS)
# Para cada classe de reparo de DNA, adicionar uma coluna correspondente em Samples_Data.1
for (class in dna_repair_classes) {
  # Nome da coluna baseado na classe
  column_name <- paste("HDRG", gsub(" ", "_", class), sep = ".")
  # Inicializar a coluna com zeros
  Samples_Data.1[[column_name]] <- 0
  # Filtrar amostras com genes na classe atual
  samplesHDRG <- unique(subset(syn.Mutation.MAF, Hugo_Symbol %in% 
                                 Human.DNA.Repair.Genes[Human.DNA.Repair.Genes$DNA_Repair.CLASS == class, ]$DNA_Repair.GENE, 
                               select = Tumor_Sample_Barcode))
  # Atualizar a coluna para 1 se a amostra contém um gene de reparo de DNA na classe
  Samples_Data.1[Samples_Data.1$EXOME_ID %in% samplesHDRG$Tumor_Sample_Barcode, column_name] <- 1
}
colnames(Samples_Data.1)[2] <- "Tumor_Sample_Barcode" 
fwrite(Samples_Data.1, paste0(output,"/Samples_Data.1.tsv"), quote = F, sep="\t")
...
for (resp in respostas) {
  for (hdrg in hdrg_columns) {
    aux1<- as.data.frame(unique(subset(Samples_Data.1, select = c("Sample", hdrg, resp))))
    dt3<- matrix(table(subset(aux1, select = c(hdrg, resp))), nr=2)
    if(ncol(dt3)>1){
      ...
      if((valor_p <= 0.05)||(valor_p1 <= 0.05)){
        ...
        print(resp)
        print(hdrg)
        print(as.matrix(table(subset(aux1, select = c(hdrg, resp))), nr=2))
        print("========== Chisq Test =======================================")
